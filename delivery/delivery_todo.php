...
if(isset($_POST['submit'])){
  ...
  $insert_order = $conn->prepare("UPDATE `orders` SET order_number = ?, payment_status = ? WHERE id = ?");
   $insert_order->execute([$order_no, $payment_status, $order_id]);
...
header("location: delivery_delivered.php");
}
...
<?php
      if($riders_user_id == ''){
         ...
         $select_orders = $conn->prepare("SELECT * FROM `orders` WHERE riders_user = ? and payment_status = 'To Receive'");
...
<div class="box">
      ...
      <form action=" " method="POST" class="boxbox">
         ...
         <input type="hidden" name="notify_rider" value="You sucessfully deliver the order">
         ...
         <input type="submit" name="submit" value="Deliver" class="btn">
      </form>
      </div>
      <?php
         }
